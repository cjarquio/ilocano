...
export const Lesson: React.FC<LessonComponentProps> = (props: LessonComponentProps) => {
  const {currentSection, lessonData, children, showDialogSection=true} = props
  const sectionNumber = sectionTypes[currentSection as keyof typeof sectionTypes]
  ...
  return (
    <Box>
      <Text fontSize={'xl'}>{lessonData.sections[sectionNumber].sectionType} {lessonData.title}</Text>
      <Text fontSize={'xl'}>{lessonData.sections[sectionNumber].description}</Text>
      <Box sx={{display: 'flex', flexDirection: 'row', padding: '0 3rem'}}>
        <Box sx={{width: '100%'}}>
          {
            showDialogSection &&  <DialogSection dialog={lessonData.dialog} dialogWords={lessonData.words_for_lesson} />
          }
        </Box>
